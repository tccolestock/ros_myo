...
from std_msgs.msg import (String, UInt8, Header, MultiArrayLayout,
...
class MyoRaw(object):
    ...
    def connect(self):
        ...
        if self.old:
            ...
            # actual framerate starts dropping inversely. Also, if this is
            ...
            self.write_attr(0x19, pack('BBBBHBBBBB', 2, 9, 2, 1, C, emg_smooth,
                            C // emg_hz, imu_hz, 0, 0))
    ...
    def add_orien_handler(self, h):
        self.orien_handlers.append(h)
...
if __name__ == '__main__':
    ...
    # Define Publishers
    imuPub = rospy.Publisher('myo_imu_left', Imu, queue_size=10)
    emgPub = rospy.Publisher('myo_emg_left', EmgArray, queue_size=10)
    armPub = rospy.Publisher('myo_arm_left', MyoArm, queue_size=10)
    gestPub = rospy.Publisher('myo_gest_left', UInt8, queue_size=10)

    orienPub = rospy.Publisher('myo_orien_left', Vector3, queue_size=10)
    # posPub=rospy.Publisher('myo_pos', Vector3, queue_size=10)

    rospy.init_node('myo_raw_left', anonymous=True)
    ...
    def proc_imu(quat1, acc, gyro):
        ...
        # define MYOHW_ORIENTATION_SCALE   16384.0f ///< See
        # myohw_imu_data_t::orientation
        # define MYOHW_ACCELEROMETER_SCALE 2048.0f  ///< See
        # myohw_imu_data_t::accelerometer
        # define MYOHW_GYROSCOPE_SCALE     16.0f    ///< See
        # myohw_imu_data_t::gyroscope
        h = Header()
        h.stamp = rospy.Time.now()
        h.frame_id = 'myo'
        # We currently do not know the covariance of the sensors with each
        # other
        cov = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        quat = Quaternion(quat1[0] / 16384.0, quat1[1] / 16384.0,
                          quat1[2] / 16384.0, quat1[3] / 16384.0)
        # Normalize the quaternion and accelerometer values
        quatNorm = \
            math.sqrt(quat.x*quat.x + quat.y*quat.y + quat.z*quat.z +
                      quat.w*quat.w)
    ...
    m.add_orien_handler(proc_orien)
